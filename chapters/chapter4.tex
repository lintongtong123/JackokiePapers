...
无线信号的调制信息主要以信号的瞬时幅度、瞬时频率和瞬时相位等特征进行表征，合理地选择和构造信号的的统计量是获得信号调制方式的重要方法。\par

\subsubsection{零中心归一化瞬时幅度谱密度的最大值}
	
$\gamma_{max}$表征了信号瞬时幅度变化的统计特性，反映了无线信号调制包络的变化情况。我们可以通过设置一定的判决门限，来区分恒定包络和非恒定包络的调制信号。\par
\begin{equation}
\label{eqt_4_2}
...
\subsubsection{零中心归一化非弱信号瞬时幅度标准差$\delta_{da}$}
\begin{equation}
\label{eqt_4_3}
...
\subsubsection{零中心归一化瞬时幅度绝对值的标准差$\delta_{aa}$}
\begin{equation}
\label{eqt_4_4}
...
\subsubsection{零中心归一化瞬时幅度的四阶紧致性$\mu_{42}^{a}$}
\begin{equation}
\label{eqt_4_5}
...
\subsubsection{零中心归一化瞬时频率均值的平方与方差之比$R_f$}
\begin{equation}
\label{eqt_4_6}
R_f = u_f^2 / d_f
\end{equation}
...
\subsubsection{零中心非弱信号段归一化瞬时频率绝对值的标准差$\delta_{af}$}
\begin{equation}
\label{eqt_4_7}
...
特征融合理论主要基于两个基本假设：
1) 多特征融合通常比单特征具备更好的分类性能；2) 融合的特征之间相关性尽量小。
...
然后，基于全局的角度对包括传统特征在内的所有特征，进行如\ref{eqt:4_18}所示的全局Normalization。\par
...
具体地说，我们的假设函数$h_{\theta}(x)$形式如方程\ref{eqt_4_19}：\par
...
那么$softmax$回归算法的损失函数有方程\ref{eqt_4_20}。
\begin{equation}
	\label{eqt_4_20}
...
我们通过添加一个权重衰减项$\frac{\lambda}{2} \sum_{i=1}^k \sum_{j=0}^{n} \theta_{ij}^2$来修改代价函数，这个衰减项会惩罚过大的参数值，现在我们的代价函数变为\ref{eqt4_21}
\begin{equation}
	\label{eqt4_21}
...
对$J(\theta)$求导，我们得到梯度方程\ref{eqt_4_22}。
\begin{align}
	\label{eqt_4_22}
...
那么利用投票法所得的分类结果如方程\ref{eqt_4_24}所示：
\begin{equation}
	\label{eqt_4_24}
