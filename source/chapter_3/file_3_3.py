...
if __name__ == '__main__':
    ...
    for i in range(len(data)):
        ...
        plt.plot(data_1_tran[0], linestyle='--')
        plt.plot(data_1_tran[1], linestyle='-.')
        axes = plt.gca()
        plt.xticks([])
        plt.yticks([])
        if i == 0:
            plt.ylabel('原始信号', fontsize=12, rotation='horizontal', labelpad=35)

        plt.subplot(2, 3, i+4)
        plt.plot(data_temp[0], linestyle='--')
        plt.plot(data_temp[1], linestyle='-.')
        axes = plt.gca()
        plt.xticks([])
        plt.yticks([])

        if i == 0:
            plt.ylabel('重构信号', fontsize=12, rotation='horizontal', labelpad=35)
